...
def _parse_dir_obj(obj: Any) -> Dict[str, Dict[str, str]]:
    ...
    Returns a dict with UPPERCASE MAC keys.
    """
...
async def _fetch_dir_from_url(hass: HomeAssistant, url: str) -> Dict[str, Dict[str, str]]:
    """Best-effort fetch of a JSON directory from URL."""
    if not url:
        return {}
    try:
        ...
        async with session.get(url, timeout=10) as resp:
            ...
            # Let aiohttp detect content-type; fall back to text->json
            text = await resp.text()
        data = json.loads(text)
        return _parse_dir_obj(data)
    except Exception as exc:  # swallow & warn; keep scanning
        _LOGGER.warning("Failed to fetch directory from %s: %s", url, exc)
        return {}


class NetworkScanner(Entity):
    """Representation of a Network Scanner (Extended)."""
    ...
    async def async_update(self) -> None:
        ...
        try:
            # Build effective directory: data (setup) + options text/url (optional)
            directory: Dict[str, Dict[str, str]] = dict(self.entry.data.get("mac_directory", {}))

            # Merge options JSON text (highest precedence)
            ...
            # Merge URL(s): options URL first, then data URL as fallback
            ...
            if url_to_use:
                fetched = await _fetch_dir_from_url(self.hass, url_to_use)
                if fetched:
                    directory.update(fetched)

            _LOGGER.debug("Effective directory size: %d", len(directory))

            # Scan network in executor (nmap is blocking)
    ...
    def _scan_network(self, directory: Dict[str, Dict[str, str]]) -> List[Dict[str, Any]]:
        ...
        for host in self.nm.all_hosts():
            try:
                ...
                if not mac or not ip:
                    continue  # need both to be useful

                # Vendor (case-insensitive lookup in nmap's vendor map)
                vendor = "Unknown"
                ven_map = self.nm[host].get("vendor", {})
                if isinstance(ven_map, dict) and ven_map:
                    # nmap keys may be upper/lower; compare upper
        ...
        # Sort devices by IPv4 numerically (fallback: lexicographic)
