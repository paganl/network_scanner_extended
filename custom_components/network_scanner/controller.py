...
class ScanController:
    ...
    @property
    def phase(self) -> str:
        return self._phase  # idle | arp | nmap

    # ---------------- internal: emit ----------------

    def _emit_update(self) -> None:
        # Fire on the HA loop; thread-safe
        self.hass.loop.call_soon(async_dispatcher_send, self.hass, SIGNAL_NSX_UPDATED)
        _LOGGER.debug(
            "Dispatched %s (status=%s, phase=%s, count=%s)",
            SIGNAL_NSX_UPDATED, self._status, self._phase, self._device_count,
        )
    ...
    async def maybe_auto_scan(self) -> None:
        ...
        try:
            last = (
                datetime.fromisoformat(self._last_scan_finished.replace("Z", "+00:00")).timestamp()
                if self._last_scan_finished else 0.0
            )
        except Exception:
            last = 0.0
    ...
    async def scan_now(self) -> None:
        if self._is_scanning:
            _LOGGER.debug("Scan suppressed: already scanning")
            return
        ...
        try:
            # -------- Phase 1: ARP (optional early publish) --------
            arp_map: Dict[str, Dict[str, Any]] = {}
            if (
                self._arp_provider == ARP_PROVIDER_OPNSENSE
                and self._opn_url and self._opn_key and self._opn_sec
            ):
                try:
            ...
            if self._nmap_args:
                ...
                if my_gen == self._scan_gen:
                    ...
                    self._phase = PHASE_ARP if arp_map else PHASE_IDLE
                    self._emit_update()
            ...
            try:
                async with session.post(
                    ...
                    try:
                        ...
                        _LOGGER.warning("OPNsense returned non-JSON from %s, first bytes=%r", url, short)
                        return {}
